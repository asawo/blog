...
The full source is on [GitHub](https://github.com/asawo/baby-monitor) if you want to run it yourself.
...
The problem is, I don't want HLS due to its inherent buffering latency. I want a real-time protocol with minimal buffering, which is exactly what `WebRTC` provides. It's not as simple as HLS but by installing `MediaMTX`, all the complexity is abstracted away.
...
The WebRTC stream includes audio, so I figured I'd visualize it too. Using the Web Audio API, I tap into the stream's audio track, route it through an `AnalyserNode`, and sample amplitude at regular intervals. The result is a scrolling bar chart that color-codes the level — green for quiet, yellow for moderate, red for loud.
...
For cry detection I used **YAMNet** — a pre-trained audio classification model from Google trained on the AudioSet dataset. It recognizes ~521 audio classes, including "Baby cry, infant cry" at class index 20.
...
Instead I applied spectral analysis: wet farts tend to have higher-frequency energy (>1kHz) while dry farts sit in the low-to-mid range (<500Hz). The classifier looks at the peak high-frequency energy ratio across overlapping frames within each audio window. Using the peak rather than the mean matters — a wet event lasting only part of the window would otherwise get diluted by surrounding silence.

It's accurate about 70% of the time. To push that further I'd need to factor in spectral entropy, flux, and waveform kurtosis — basically how messy and variable the waveform is — since wet farts have more pops, spikes, and moment-to-moment variation than dry ones.
...
One thing that bit me early on was USB device path instability. Paths like `/dev/video0` can shift on reboot depending on the order the kernel enumerates USB devices. The fix was a udev rule that matches the webcam by its USB vendor/product ID and creates a stable symlink at `/dev/baby-cam`. Now the device path never changes regardless of what else is plugged in.

Similarly, ALSA audio card numbers can shift, so instead of addressing the mic by card number I address it by card name (`hw:WEBCAM,0`), which is derived from the USB descriptor and stays consistent.

Everything runs as systemd services on the Pi — the FFmpeg stream, MediaMTX, the Go HTTP server, and the Python detection script. The stream service depends on the camera device unit, so systemd waits for the camera to appear before starting and automatically stops the service if it gets unplugged.
